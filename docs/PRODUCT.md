...
- **Pieces in one set** is explicit product packaging data. Products with missing packaging or Stock/set that does not equal QTY divided by Pieces in one set appear under **Needs setup** and cannot be sold until corrected.
...
4. Record a sale in whole sets, loose pieces, or both. Set-priced CP, SRP, and MRP values are divided proportionally by Pieces in one set.
   Record a sale displays both saved stock values and subtracts sold pieces proportionally from each; it never silently replaces Stock/set with a derived value.
